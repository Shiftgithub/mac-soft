...
import os
import sys  # help with importing images
...
class Board(tk.Frame):
    ...
    def __init__(self, parent, length, width):  # self=Frame, parent=root
        ...
        self.white_pieces = ["pyimage1", "pyimage3",
                             "pyimage4", "pyimage5", "pyimage6", "pyimage7"]
        self.black_pieces = ["pyimage8", "pyimage10",
                             "pyimage11", "pyimage12", "pyimage13", "pyimage14"]
    ...
    def select_piece(self, button):
        ...
        if (self.piece_color == "white" and self.turns % 2 == 0) or (self.piece_color == "black" and self.turns % 2 == 1) or self.buttons_pressed == 1:
            if self.buttons_pressed == 0:  # stores square and button of first square selected
                self.sq1 = list(self.squares.keys())[list(
                    self.squares.values()).index(button)]  # retrieves pos of piece
                self.sq1_button = button
                self.buttons_pressed += 1

            elif self.buttons_pressed == 1:  # stores square and button of second square selected
                self.sq2 = list(self.squares.keys())[
                    list(self.squares.values()).index(button)]
                ...
                if self.allowed_piece_move() and self.friendly_fire() == False:  # makes sure the move is legal
                    ...
                    self.squares[self.sq2].config(
                        image=self.sq1_button["image"])  # moves pice in sq1 to sq2
                    self.squares[self.sq2].image = self.sq1_button["image"]
                    self.squares[self.sq1].config(
                        image=self.white_images["blank.png"])  # clears sq1
                    self.squares[self.sq1].image = self.white_images["blank.png"]
                    # for some reason it says king is in check after a castle so I set up a variable here that would prevent this code from running
                    if self.in_check() == True and self.castled == False:
                        # reverts movement since king is or would be put into check because of move
                        self.squares[prev_sq2].config(
                            image=prev_sq2_button_piece)
                        self.squares[prev_sq2].image = prev_sq2_button_piece
                        self.squares[prev_sq1].config(
                            image=prev_sq1_button_piece)
    ...
    def promotion_menu(self, color):
        ...
        if color == "white":
            promo_knight = tk.Button(promo, text="Knight", command=lambda: return_piece(
                "pyimage4"))  # triggers return_piece function when selected
            promo_knight.grid(row=0, column=0)
            promo_bishop = tk.Button(
                promo, text="Bishop", command=lambda: return_piece("pyimage1"))
            promo_bishop.grid(row=0, column=1)
            promo_rook = tk.Button(
                promo, text="Rook", command=lambda: return_piece("pyimage7"))
            promo_rook.grid(row=1, column=0)
            promo_queen = tk.Button(
                promo, text="Queen", command=lambda: return_piece("pyimage6"))
            promo_queen.grid(row=1, column=1)
        elif color == "black":
            promo_knight = tk.Button(
                promo, text="Knight", command=lambda: return_piece("pyimage11"))
            promo_knight.grid(row=0, column=0)
            promo_bishop = tk.Button(
                promo, text="Bishop", command=lambda: return_piece("pyimage8"))
            promo_bishop.grid(row=0, column=1)
            promo_rook = tk.Button(
                promo, text="Rook", command=lambda: return_piece("pyimage14"))
            promo_rook.grid(row=1, column=0)
            promo_queen = tk.Button(
                promo, text="Queen", command=lambda: return_piece("pyimage13"))
            promo_queen.grid(row=1, column=1)
        promo.mainloop()
        return
        ...
        if piece == "rook" or piece == "queen":
            if self.sq1[0] == self.sq2[0]:  # for vertical movement
                ...
                for i in range(pos1+1, pos2):
                    square_on_path = self.squares[self.sq1[0] +
                                                  str(i)].cget("image")
                    if square_on_path != "pyimage2":
                        return False

            elif self.sq1[1] == self.sq2[1]:  # for horizontal movement
                pos1 = min(self.ranks.find(
                    self.sq1[0]), self.ranks.find(self.sq2[0]))
                pos2 = max(self.ranks.find(
                    self.sq1[0]), self.ranks.find(self.sq2[0]))

                for i in range(pos1+1, pos2):
                    square_on_path = self.squares[self.ranks[i] +
                                                  self.sq1[1]].cget("image")
                    if square_on_path != "pyimage2":
                        return False
            ...
            if y1 < y2:
                if x1 < x2:  # NE direction
                    for x in range(x1+1, x2):
                        y1 += 1
                        square_on_path = self.squares[self.ranks[x] +
                                                      str(y1)].cget("image")
                        if square_on_path != "pyimage2":
                            return False
        ...
        if self.sq1_button["image"] == wp:  # white pawn movement
            if "2" in self.sq1:  # allows for 2 space jump from starting pos
                if (int(self.sq1[1])+1 == int(self.sq2[1]) or int(self.sq1[1])+2 == int(self.sq2[1])) and self.sq1[0] == self.sq2[0] and self.sq2_button["image"] == "pyimage2":  # allows 2 sq movement
                    in_front = self.squares[self.sq1[0] +
                                            str(int(self.sq1[1])+1)]
    ...
    def castle(self):  # checks to see if the move entails a castle, and if a castle is allowed
        if self.wk_moved == False:  # makes sure king hasn't moved
            # finds out which way user wants to castle and if the rook has moved (in this case white would want to castle to the left)
            if self.wr1_moved == False and self.sq2 == "c1":
                # checks to see if squares in between rook and king are empty and are not a possible move for opponent
                for x in range(1, 4):
    ...
    def set_squares(self):  # fills frame with buttons representing squares
        ...
        for x in range(8):
            for y in range(8):
                ...
                B = tk.Button(self, bg=self.square_color,
                ...
                self.squares[pos].config(
                    command=lambda key=self.squares[pos]: self.select_piece(key))

    # opens and stores images of pieces and prepares the pieces for the game for both sides
    ...
    def set_pieces(self):  # places pieces in starting positions
        ...
        dict_rank2_pieces = {"a2": "p.png", "b2": "p.png", "c2": "p.png",
                             "d2": "p.png", "e2": "p.png", "f2": "p.png", "g2": "p.png", "h2": "p.png"}
        dict_rank7_pieces = {"a7": "p.png", "b7": "p.png", "c7": "p.png",
                             "d7": "p.png", "e7": "p.png", "f7": "p.png", "g7": "p.png", "h7": "p.png"}
        dict_rank8_pieces = {"a8": "r.png", "b8": "n.png", "c8": "b.png",
                             "d8": "q.png", "e8": "k.png", "f8": "b.png", "g8": "n.png", "h8": "r.png"}
        ...
        for rank in range(3, 7):  # fill rest with blank pieces
            for file in range(8):
                ...
                self.squares[pos].config(
                    image=self.white_images[starting_piece])
